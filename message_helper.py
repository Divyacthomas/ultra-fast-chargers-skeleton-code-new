...
class ModuleMessage:
    ...
    def digital_output_open_fan(cls):
        message = can.Message(arbitration_id=CanId.DIGITAL_OUT,
                              is_extended_id=False, data=[0, 0, 64, 0])  # data changed for 240kW
        cls.bus.send(message)
    @classmethod
    def digital_output_led_red(cls):
        message=can.Message(arbitration_id=CanId.DIGITAL_OUT,
                          is_extended_id=False, data=[128, 0, 128, 0])       # data changed for 240kW
        cls.bus.send(message)

    @classmethod
    def digital_output_led_green(cls):
        message=can.Message(arbitration_id=CanId.DIGITAL_OUT,
                          is_extended_id=False, data=[0, 1, 0, 1])       # data changed for 240kW
        cls.bus.send(message)
    
    @classmethod
    def digital_output_led_blue(cls):
        message=can.Message(arbitration_id=CanId.DIGITAL_OUT,
                          is_extended_id=False, data=[0, 2, 0, 2])       # data changed for 240kW
        cls.bus.send(message)
    ...
    def setCurrent_b(cls, can_ids):
        global_data = ConstantManager240KW()
        tmp_current2 = DTH.convertohex(global_data.get_data_running_current2())

        if not isinstance(can_ids, list):
            can_ids = [can_ids]
        for can_id in can_ids:
            message = can.Message(arbitration_id=can_id, is_extended_id=True, data=[32, 3, 0, 0, 0] + tmp_current2)
            cls.bus.send(message)
    ...
    @classmethod
    def readModule_Current_a(cls, can_ids):
        if not isinstance(can_ids, list):
            can_ids = [can_ids]
        for can_id in can_ids:
            message = can.Message(arbitration_id=can_id, is_extended_id=True, data=[
    ...
    @classmethod
    def readModule_Current_b(cls, can_ids):
        if not isinstance(can_ids, list):
            can_ids = [can_ids]
        for can_id in can_ids:
            message = can.Message(arbitration_id=can_id, is_extended_id=True, data=[
    ...
    def pecc_powers_voltage_limits(cls, can_id):
        message = can.Message(arbitration_id=can_id,
                              is_extended_id=False, data=[220, 5, 16, 39, 88, 2, 0, 0])
        cls.bus.send(message)

    @classmethod
    def pecc_current_limits(cls, can_id):
        message = can.Message(arbitration_id=can_id,
                              is_extended_id=False, data=[0, 0, 100, 0])
        cls.bus.send(message)
...
class Module1Message(ModuleMessage):
    ...
    @classmethod
    def digital_output_ACclose_Gun1(cls):
        message = can.Message(arbitration_id=CanId.DIGITAL_OUT,
                              is_extended_id=False, data=[16, 0, 16, 0])  # data changed for 240kW
        cls.bus.send(message)

    @classmethod
    def digital_output_ACopen_Gun1(cls):
        message = can.Message(arbitration_id=CanId.DIGITAL_OUT,
    ...
    def digital_output_close_Gun1(cls):
        message = can.Message(arbitration_id=CanId.DIGITAL_OUT,
                              is_extended_id=False, data=[67, 0, 67, 0])  # data changed for 240kW
        cls.bus.send(message)

    @classmethod
    def digital_output_open_stop1(cls):
        message = can.Message(arbitration_id=CanId.DIGITAL_OUT,
                              is_extended_id=False, data=[0, 0, 3, 0])  # data changed for 240kW
        cls.bus.send(message)
...
class Module2Message(ModuleMessage):
    ...
    @classmethod
    def digital_output_ACclose_Gun2(cls):
        message = can.Message(arbitration_id=CanId.DIGITAL_OUT,
                              is_extended_id=False, data=[32, 0, 32, 0])  # data changed for 240kW
        cls.bus.send(message)

    @classmethod
    def digital_output_ACopen_Gun2(cls):
        message = can.Message(arbitration_id=CanId.DIGITAL_OUT,
                              is_extended_id=False, data=[0, 0, 32, 0])  # data changed for 240kW
        cls.bus.send(message)
   
    @classmethod
    def digital_output_close_Gun2(cls):
        message = can.Message(arbitration_id=CanId.DIGITAL_OUT,
                              is_extended_id=False, data=[76, 0, 76, 0])  # data changed for 240kW
        cls.bus.send(message)

    @classmethod
    def digital_output_open_stop2(cls):
        message = can.Message(arbitration_id=CanId.DIGITAL_OUT,
                              is_extended_id=False, data=[0, 0, 12, 0])  # data changed for 240kW
        cls.bus.send(message)
